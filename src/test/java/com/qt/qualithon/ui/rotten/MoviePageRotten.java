 ...
 **/
public class MoviePageRotten extends Page{

    public MoviePageRotten(TestSession testSession){
        super(testSession);
        ...
        for(WebElement credit:credits){
            try{
                if(credit.findElement(By.cssSelector("div.meta-label.subtle")).getText()
                		.replaceAll(":", "").equalsIgnoreCase("Director")){
    ...
    public List<String> genres(){
        ...
        List<WebElement> credits = this.testSession.driverWait().until(
                ExpectedConditions.presenceOfAllElementsLocatedBy(
                		By.cssSelector("ul.content-meta.info li")));
        for(WebElement credit:credits){
            try{ if(credit.findElement(By.cssSelector("div.meta-label.subtle"))
    ...
    public String releaseYear(){
        String movieInfo = this.testSession.driverWait().until(
            ExpectedConditions.presenceOfElementLocated(
                By.xpath("//p[@class='scoreboard__info']"))
            ) 
        .getText();
